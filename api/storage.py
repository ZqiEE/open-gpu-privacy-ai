...
class SchedulerStore:
    ...
    def queued_route_preview(self, node_id: str, limit: int = 20) -> dict:
        ...
        with self.connect() as conn:
            rows = conn.execute("SELECT * FROM jobs WHERE status = 'queued' ORDER BY created_at ASC LIMIT ?", (limit,)).fetchall()
    ...
    def next_job(self, node_id: str) -> dict | None:
        ...
        if not node:
            return None
        with self.connect() as conn:
            rows = conn.execute("SELECT * FROM jobs WHERE status = 'queued' ORDER BY attempts ASC, created_at ASC LIMIT 50").fetchall()
        ...
        job = self.get_job(chosen["job_id"])
