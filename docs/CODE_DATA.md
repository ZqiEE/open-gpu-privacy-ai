...
Training output does not automatically become a live runtime. A local artifact is first bound as `candidate`, then the promotion gate checks artifact integrity, basic training metrics, code-record presence, Python syntax checks for code samples, executable code-generation eval support, and distributed replica health. Owned chat/runtime paths only load `active` bindings, so failed candidates remain inspectable but are not served.

Failed candidates now create structured follow-up work. Replica/distribution failures are handled by replica repair. Model-quality and code-quality failures create a queued retrain action with the original dataset URI, source job id, binding id, and gate blockers. The local worker submits those actions back into `/training/jobs`, closing the first automatic repair/retrain loop for failed training artifacts. The `lightweight-ngram` fallback intentionally fails executable code-generation promotion because it cannot write tested code; it is a pipeline bootstrap, not the final code model.
