...
Candidate artifacts that fail the promotion gate write `runtime_data/candidate_failure_actions.json`. Model-quality failures become `training_retrain` actions that the worker submits back into `/training/jobs`; storage/replica failures stay in the replica repair path. This keeps automatic training moving after bad candidates instead of requiring manual triage.
