...
def test_rollback_marks_binding_rolled_back(tmp_path) -> None:
    ...
    runtime_model = runtime.register_model_from_dict if False else None
    from api.runtime_router import ModelManifest

    runtime.register_model(ModelManifest(model_id="ailovanta-owned", version="candidate", manifest_hash="sha256:r", privacy_level="protected", min_gpu_memory_gb=0, allowed_pools=["trusted_runtime_pool"], quantization="candidate", context_length=8192, adapter_compatible=True, status="active"))
    bindings.register_binding({"model_id": "ailovanta-owned", "version": "candidate", "model_key": "ailovanta-owned:candidate", "manifest_hash": "sha256:r", "status": "active"}, {"artifact_id": "artifact_1", "artifact_hash": "sha256:a", "checkpoint_uri": "file:///tmp/checkpoint.bin"}, status="active")
